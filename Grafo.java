...
public class Grafo {
	...
	private Map<BigInteger,Nodo> elenco; //dizionario dei nodi esistenti
	...
	public Grafo() {
			...
			this.elenco = new Hashtable<BigInteger,Nodo>();
	}	
	
	//elenco hashtable
	public Map<BigInteger,Nodo> getElenco() {
		return this.elenco;
	}
	
	public void setElenco(BigInteger n, Nodo v) {
		this.elenco.put(n, v);
	}
	...
	public Nodo trovaV(BigInteger v) {
		Nodo n=null;
	...
	*/
	...
	public void random(int n) {
		...
		BigInteger v=new BigInteger(n, random);
		BigInteger w=new BigInteger(n, random);
		...
		System.out.println("listanodi"+ this.getListaNodi());					
	}
	...
	public void random1(int v,int w) {
		...
		System.out.println("creo l'arco ("+v+","+w+")");
		Nodo nodov = this.trovaV(bv);
		if(nodov==null) {
			nodov =new Nodo(bv,null);
			this.setElenco(bv,nodov);//
			this.addListaNodi(nodov);
		}
		
		Nodo nodow = this.trovaV(bw);
		
		if(nodow==null) {
		
			nodow = new Nodo(bw,nodov);
			this.setElenco(bw,nodow);//
			this.ordinaLista(nodow);
			nodov.addOutLink(nodow);			
		}else if(nodow.getInLink()!=nodov && nodov!=nodow){
				this.processBacklink(nodov,nodow); 	}
